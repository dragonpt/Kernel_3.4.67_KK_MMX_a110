...
//Ivan added
	...
	data = mt_get_gpio_in(GPIO_ALS_EINT_PIN);
	if (data & 0x01)
	    mt_eint_set_polarity(CUST_EINT_ALS_NUM, CUST_EINT_POLARITY_LOW);
	else	    
	    mt_eint_set_polarity(CUST_EINT_ALS_NUM, CUST_EINT_POLARITY_HIGH);
...
//	struct cm3607_priv *obj = i2c_get_clientdata(client);        
	...
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, cm3607_eint_func, 0); 
	mt_eint_unmask(CUST_EINT_ALS_NUM);  
#endif  
#ifdef MT6575
